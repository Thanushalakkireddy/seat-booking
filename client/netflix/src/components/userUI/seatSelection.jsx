...
export default function SeatSelection() {
    ...
    useEffect(() => {
        const fetchMovieAndShows = async () => {
            try {
                // Fetch Movie
                const movieRes = await axios.get(`http://your-public-ip:5000/api/user/viewMovie/${id}`);
                setMovie(movieRes.data);

                // Fetch Shows
                const showsRes = await axios.get(`http://your-public-ip:5000/api/user/shows/${id}`);
    ...
    const fetchBookedSeats = async (showId) => {
        if (!showId) return;
        try {
            const response = await axios.get(`http://your-public-ip:5000/api/user/booked-seats/${showId}`);
    ...
    const createTempBooking = async (seat) => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://your-public-ip:5000/api/user/temp-booking',
                {
    ...
    const cancelTempBooking = async (seatId) => {
        try {
            ...
            // Cancel the booking
            await axios.post(
                'http://your-public-ip:5000/api/user/cancel-booking',
                {
                    showId: selectedShow.id,
                    seats: [seatId]
                },
    ...
    const handleBooking = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://your-public-ip:5000/api/user/book',
                {
                    showId: selectedShow.id,
