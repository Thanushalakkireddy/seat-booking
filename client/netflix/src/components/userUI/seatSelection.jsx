...
export default function SeatSelection() {
    ...
    useEffect(() => {
        const fetchMovieAndShows = async () => {
            try {
                // Fetch Movie
                const movieRes = await axios.get(`${API_BASE_URL}/api/user/viewMovie/${id}`);
                setMovie(movieRes.data);

                // Fetch Shows
                const showsRes = await axios.get(`${API_BASE_URL}/api/user/shows/${id}`);
    ...
    const fetchBookedSeats = async (showId) => {
        if (!showId) return;
        try {
            const response = await axios.get(`${API_BASE_URL}/api/user/booked-seats/${showId}`);
    ...
    const createTempBooking = async (seat) => {
        ...
        try {
            ...
            const response = await axios.post(
                '${API_BASE_URL}/api/user/temp-booking',
                {
    ...
    const cancelTempBooking = async (seatId) => {
        try {
            ...
            // Cancel the booking
            await axios.post(
                '${API_BASE_URL}/api/user/cancel-booking',
                {
                    showId: selectedShow.id,
                    seats: [seatId]
                },
